...
CONTROL_HORIZON = 15 #5  # seconds. if this is set too high and INTEGRATION_DT is too low, code will take a long time to run!
INTEGRATION_DT = 1.5 #.025  # s, delta t to propagate trajectories forward by
...
class PathFollower():
    def __init__(self):
        ...
        # goal
        self.cur_goal = np.array(self.path_tuples[1])
    ...
    def points_to_robot_circle(self, points):
        ...
        rob_locations = np.rint(points).astype(int)
        rob_radius = self.collision_radius_pix #Get the robot radius in pixel form
        rr = [[] for x in rob_locations]
        cc = [[] for x in rob_locations]
        for idx, center in enumerate(rob_locations):
            rr[idx], cc[idx] = circle(center[0], center[1], int(np.ceil(rob_radius)))
        return rr, cc
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            for opt in range(0, self.num_opts):
                #for the horizon timesteps, rollout all trajectory options shown in all_options
                #for each t, there will be 43 trajectories of size horizon timesteps (x,y, theta)
                #all opts is 43 x 2 , 2 represents a vel and rot vel
                for t in range(1, self.horizon_timesteps + 1):
                    ...
                    vels = self.all_opts[opt]
                    vel = vels[0] #extract vel
                    rot_vel = vels[1] #extract rot vel
                    p = np.array([[vel, rot_vel]]).T
                    #local_paths[t,opt,:] = trajectory_rollout(vel,rot_vel)
                    #sub_step = self.timestep / INTEGRATION_DT #self.num_substeps

                    G = np.array([[np.cos(local_paths[t-1, opt, 2]), 0],
                                  [np.sin(local_paths[t-1, opt, 2]), 0],
                                  [0, 1]])
                    q_dot = np.matmul(G,p)
                    local_paths[t, opt, :] = local_paths[t-1, opt, :] + (q_dot*INTEGRATION_DT).T

            # check all trajectory points for collisions
            # first find the closest collision point in the map to each local path point
            local_paths_pixels = (self.map_origin[:2] + local_paths[:, :, :2]) / self.map_resolution
            local_paths_lowest_collision_dist = np.ones(self.num_opts) * 50
            collision_table = np.ones(self.num_opts) #table of collision checks

            trajs = []
            for opt in range(local_paths_pixels.shape[1]): #th options in local path pizels
                #for timestep in range(local_paths_pixels.shape[0]): #for every timesteps
                traj = local_paths_pixels[:,opt,:]
                trajs += [traj]
                assert(traj.shape[1] == 2) #DEBUG
                rr, cc = self.points_to_robot_circle(traj) #Get list of pixels the robot will pass through
                ...
                is_collision = self.map_np[tuple(path)] #Index occupany map at this position
                if np.sum(is_collision) > 0:
                    collision_table[opt] = 0  #set to 0 to indicate collision for this option and timestep

            # if np.sum(collision_table) > -1:
            #     plt.imshow(self.map_np, cmap = 'Greys', interpolation = 'nearest')
            #     # plt.plot(self.pose_in_map_np[0], self.pose_in_map_np[1], Marker = 'x', color = 'red')
            #     for i, traj in enumerate(trajs):
            #         plt.plot(traj[:,0], traj[:,1], color = 'blue')
            #         if collision_table[i] == 0:
            #             plt.plot(traj[-1,0], traj[-1,1], Marker = 'o', color = 'red')
            #         else:
            #             plt.plot(traj[-1,0], traj[-1,1], Marker = 'o', color = 'green')
            #     plt.show()

            ####ADDED -> determine final cost
            final_cost = np.ones(self.num_opts)*np.inf
            valid_opts = np.where( collision_table == 1 )[0]
            for opt in valid_opts:
                dist_from_goal = np.linalg.norm(self.pose_in_map_np[:2] - self.cur_goal[:2])
                if dist_from_goal > 0 : #DEBUG FIX THRESHOLD
                    # print("goal: ", self.cur_goal[:2])
                    diff = local_paths[:, opt, :2] - self.cur_goal[:2]
                    dist = np.linalg.norm(diff, axis=1)
                    final_cost[opt] = np.mean(dist)

                else:
                    abs_angle_diff = np.abs(self.pose_in_map_np[2] - self.cur_goal[2])
                    final_cost[opt] = min(np.pi * 2 - abs_angle_diff, abs_angle_diff)
